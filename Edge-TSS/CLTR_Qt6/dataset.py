...
class listDataset(Dataset):
    def __init__(self, root, shape=None, shuffle=True, transform=None, train=False, seen=0, batch_size=1, num_workers=4, args=None):
        if train:
            random.shuffle(root)
    ...
    def __getitem__(self, index):
        ...
        kpoint = kpoint.copy() # [683, 1024]
        img = img.copy() # [460, 998, 3]
        if self.transform is not None:
            img = self.transform(img) # [3, 512, 767]
        if self.train == True:
            count_none = 0
            imgs = []
            targets = []
            for l in range(self.args['num_patch']):
                while True:
                    ...
                    gt_points = np.nonzero(torch.from_numpy(sub_kpoint)) # [41, 2]
                    distances = self.caculate_knn_distance(gt_points, num_points) # [41, 1]
                    points = torch.cat([gt_points, distances], dim=1) # [460, 695, 3]
            ...
            img_return = img.view(3, num_h, self.args['crop_size'], width).view(3, num_h, self.args['crop_size'], num_w, self.args['crop_size'])
            img_return = img_return.permute(0, 1, 3, 2, 4).contiguous().view(3, num_w * num_h, self.args['crop_size'], self.args['crop_size']).permute(1, 0, 2, 3)
            kpoint_return = kpoint.view(num_h, self.args['crop_size'], width).view(num_h, self.args['crop_size'], num_w, self.args['crop_size'])
            kpoint_return = kpoint_return.permute(0, 2, 1, 3).contiguous().view(num_w * num_h, 1, self.args['crop_size'], self.args['crop_size'])
            targets = []
