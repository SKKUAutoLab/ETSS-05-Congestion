...
def generate_chart_image(pred_counts, gt_counts, total_frames, chart_width, chart_height):
    ...
    ax.plot(range(len(pred_counts)), pred_counts, color='black', linewidth=2, label='Pred Count')
    ax.plot(range(len(gt_counts)), gt_counts, color='blue', linewidth=2, label='GT Count')
    ax.set_xlim(0, max(total_frames, 1))
    ax.set_ylim(0, y_max)
    ax.set_xticks(h_ticks)
    ax.set_yticks(v_ticks)
    ax.set_xticklabels([''] + [str(tick) for tick in h_ticks[1:]])
    ...
    def run(self):
        try:
            ...
            for idx, (fname, img, kpoint, targets, patch_info) in enumerate(test_loader):
                ...
                total += 1
                info = '[{}/{}], Frame: {}, GT Count: {:.1f}, Pred Count: {:.1f}'.format(idx + 1, len(test_loader), fname[0], gt_count, count)
                ...
                submission_rows.append([fname[0], float(count), float(gt_count)])
            ...
            save_csv(SUBMISSION_PATH, ['path', 'total_count', 'gt_count'], submission_rows)
            save_csv(METRICS_PATH, ['MAE', 'RMSE', 'Accuracy'], [[f'{final_mae:.2f}', f'{final_rmse:.2f}', f'{accuracy:.2f}']])
            self.log.emit('MAE: {:.4f}, RMSE: {:.4f}, Accuracy: {:.2f}%'.format(final_mae, final_rmse, accuracy))
            self.log.emit('Saved submission: {}'.format(SUBMISSION_PATH))
            self.log.emit('Saved metrics: {}'.format(METRICS_PATH))
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Traffic Congestion Detection')
        ...
        self.metrics_label = QtWidgets.QLabel('The metrics will appear here after testing finishes.')
