...
def save_checkpoint(state, visi, is_best, save_path, filename='checkpoint.pth'):
    checkpoint_path = os.path.join('.', str(save_path), filename)
    checkpoint_tmp_path = checkpoint_path + '.tmp'
    torch.save(state, checkpoint_tmp_path)
    os.replace(checkpoint_tmp_path, checkpoint_path)
    if is_best:
        best_path = os.path.join('.', str(save_path), 'model_best.pth')
        best_tmp_path = best_path + '.tmp'
        shutil.copyfile(checkpoint_path, best_tmp_path)
