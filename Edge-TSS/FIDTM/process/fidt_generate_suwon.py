...
def main(args):
    ...
    if not os.path.exists(part_train.replace('images', 'gt_fidt_map')):
        os.makedirs(part_train.replace('images', 'gt_fidt_map'))
    if not os.path.exists(part_test.replace('images', 'gt_fidt_map')):
        os.makedirs(part_test.replace('images', 'gt_fidt_map'))
    ...
    for img_path in img_paths:
        ...
        Gt_data = mat["image_info"][0][0][0][0][0] # [17, 2]
        fidt_map1 = fidt_generate1(Img_data, Gt_data, 1) # [480, 640]
        kpoint = np.zeros((Img_data.shape[0], Img_data.shape[1])) # [480, 640]
        ...
        fidt_map1 = cv2.applyColorMap(fidt_map1, 2) # [480, 640, 3]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', type=str, default='datasets/Suwon')
