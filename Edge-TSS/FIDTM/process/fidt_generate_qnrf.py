...
def main(args):
    ...
    save_train_gt_path = os.path.join(args.input_dir, 'train_data/gt_fidt_map/')
    save_test_img_path = os.path.join(args.input_dir, 'test_data/images/')
    save_test_gt_path = os.path.join(args.input_dir, 'test_data/gt_fidt_map/')
    ...
    for k in range(len(img_train)):
        ...
        fidt_map = fidt_generate1(Img_data, Gt_data, 1) # [1363, 2048]
        kpoint = np.zeros((Img_data.shape[0], Img_data.shape[1])) # [1363, 2048]
