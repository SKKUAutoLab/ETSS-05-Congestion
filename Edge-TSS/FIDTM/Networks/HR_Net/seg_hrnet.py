...
class Bottleneck(nn.Module):
    ...
    def forward(self, x): # [16, 64, 64, 64]
        residual = x
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
        ...
        if self.downsample is not None:
            residual = self.downsample(x)
        out += residual
        out = self.relu(out) # [16, 256, 64, 64]
        return out
...
class HighResolutionModule(nn.Module):
    ...
    def forward(self, x): # [16, 48, 64, 64], [16, 96, 32, 32]
        if self.num_branches == 1:
        ...
        return x_fuse # [16, 48, 64, 64], [16, 96, 32, 32]
    ...
    def init_weights(self, pretrained='', train=False):
        ...
        if train == True:
            if os.path.isfile(pretrained):
                ...
                logger.info('Load ckpt from: {}'.format(pretrained))
            else:
                print('No ckpt found at: {}'.format(pretrained))
                exit()
