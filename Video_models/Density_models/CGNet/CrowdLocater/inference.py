...
from utils import get_local_rank, get_local_size, is_main_process, set_randomseed, init_distributed_mode, get_rank
...
def inference_transform():
    return A.Compose([A.Resize(768,1280), A.Normalize()])
...
class CountingDateSet(CocoDetection):
    ...
    def __getitem__(self, index):
        ...
        h_pad = h2 - h1
        w_pad = w2 - w1
        image_pad = F.pad(image, pad=(0, w_pad, 0, h_pad)) # [3, 768, 1280]
...
def inference(model, data_loader, args):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(data_loader):
            inputs = inputs.to(args.gpu) # [1, 3, 768, 1280]
            assert inputs.shape[0] == 1
            if args.distributed:
                pred_pts, pred_maps = model.module.forward_points(inputs)
            else:
                pred_pts, pred_maps = model.forward_points(inputs) # pred_maps: [1, 1, 192, 320]
...
def main(args):
    ...
    dataset_test = CountingDateSet(prefix, ann_file, max_len=args.max_len, transforms=inference_transform(), cache_mode=False, local_rank=get_local_rank(), local_size=get_local_size())
    sampler_test = DistributedSampler(dataset_test, shuffle=False) if args.distributed else None
    loader_val = DataLoader(dataset_test, batch_size=args.batch_size, sampler=sampler_test, shuffle=False, num_workers=args.num_workers, pin_memory=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="fidtm_sense.json", type=str)
    parser.add_argument("--mode", default="test", type=str)
    parser.add_argument("--draw_dir", default="../locater/vis", type=str)
    parser.add_argument("--output_dir", default="../locater/results", type=str)
    # testing config
    parser.add_argument("--vis", action="store_true")
    parser.add_argument("--ckpt_dir", default="ckpts/weight.pth")
