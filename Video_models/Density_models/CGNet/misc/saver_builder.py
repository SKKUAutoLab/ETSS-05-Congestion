...
class Saver():
    ...
    def save(self, model, optimizer, scheduler, filename, epoch, stats={}):
        if is_main_process():
            torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'scheduler': scheduler.state_dict(), 'epoch': epoch, 'states': stats}, os.path.join(self.save_dir, filename))
