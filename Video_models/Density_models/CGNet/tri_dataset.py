...
class VideoDataset(VisionDataset):
    def __init__(self, root: str, annotation_dir:str,transforms: Callable[..., Any] | None = None, transform: Callable[..., Any] | None = None,
        ...
        for video_path in self.video_paths:
            ...
            self.videos.append({"video_name": video_name, "img_names": [], "height": self.annotation[video_name][0]["height"], "width": self.annotation[video_name][0]["width"],
                                "ids": [], "pts": [], "bboxes": [], "cnt": []})
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        labels={"h": height, "w": width, "pts": pts, "bboxes": bboxes, "cnt": cnt, "video_name": video_name, "img_names": img_names}
        return imgs,labels

class PairDataset(VisionDataset):
    def __init__(self, root: str, annotation_dir:str, max_len:int ,transforms: Callable[..., Any] | None = None, transform: Callable[..., Any] | None = None,
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if self.train and (fused_num < 2 or fused_num > 30 or len(independ0_list)==0 or len(independ1_list)==0):
            return self.__getitem__((index + 1) % len(self))
        independ_pts0 = -1 * np.ones((self.max_len, 2)) # [3000, 2]
        independ_pts0[:len(independ0_list)] = np.array(independ0_list)
        independ_pts1 = -1 * np.ones((self.max_len, 2)) # [3000, 2]
        independ_pts1[:len(independ1_list)] = np.array(independ1_list)
        x = torch.cat([img0, img1], dim=0) # [6, 720, 1280]
        ...
        ref_pts = torch.stack([fused_pts0, fused_pts1], dim=0) # [2, 3000, 2]
        labels = {"h": pair["0"]["height"], "w": pair["0"]["width"], "gt_default_pts": pt_0, "gt_duplicate_pts": pt_1, "gt_fuse_pts0": fused_pts0, "gt_fuse_pts1": fused_pts1,
                  "gt_default_num": pair["0"]["cnt"], "gt_duplicate_num": pair["1"]["cnt"], "gt_fuse_num": fused_num, "video_name": video_name, "img_name1": img_name1, "img_name2": img_name2}
        inputs = {"image_pair": x, "ref_pts": ref_pts, "ref_num": fused_num, "independ_pts0": torch.from_numpy(independ_pts0).float(),
                  "independ_pts1": torch.from_numpy(independ_pts1).float(), "independ_num0": len(independ0_list), "independ_num1": len(independ1_list)}
