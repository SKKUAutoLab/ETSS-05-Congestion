...
def locator_dataset(inputs, train=False, flip=False):
    if train:
...
def process_data(img, points, train=False, flip=False, patch_size=256):
    if train:
        scale_range = [0.8, 1.2]
...
def trans_dataset(inputs, image_set):
    if image_set == 'train':
        samples, targets = locator_dataset(inputs, train=True, flip=True)
        return samples, targets
    elif image_set == 'val':
        samples, targets = locator_dataset(inputs, train=False)
        return samples, targets