...
def video_transform():
    return A.Compose([A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), ToTensorV2()]) # only use for testing
    # return A.Compose([A.Resize(720, 1280), A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), ToTensorV2()]) # use for training + testing
...
class VideoDataset(VisionDataset):
    def __init__(self, root: str, annotation_dir: str, transforms: Callable[..., Any] | None = None, transform: Callable[..., Any] | None = None,
        ...
        for video_path in self.video_paths:
            ...
            self.videos.append({"video_name": video_name, "img_names": [], "height": self.annotation[video_name][0]["height"], "width": self.annotation[video_name][0]["width"],
                                "ids": [], "pts": [], "bboxes": [], "cnt": [], "video_path": video_path})
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        labels = {"h": height, "w": width, "pts": pts, "bboxes": bboxes, "cnt": cnt, "video_name": video_name, "img_names": img_names, "video_path": video_path}
        return imgs, labels
...
class PairDataset(VisionDataset):
    def __init__(self, root: str, annotation_dir: str, max_len: int, transforms: Callable[..., Any] | None = None, transform: Callable[..., Any] | None = None,
                 target_transform: Callable[..., Any] | None = None, train=True, step=15, interval=1, force_last=False) -> None:
        super().__init__(root, transforms, transform, target_transform)
        self.video_paths = os.listdir(root)
        self.video_paths = [os.path.join(root, video_path) for video_path in self.video_paths]
        self.annotations_paths = os.listdir(annotation_dir)
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        labels = {"h": pair["0"]["height"], "w": pair["0"]["width"], "gt_fuse_pts0": fused_pts0, "gt_fuse_pts1": fused_pts1, "gt_default_num": pair["0"]["cnt"],
                  "gt_duplicate_num": pair["1"]["cnt"], "gt_fuse_num": fused_num, "video_name": video_name, "img_name1": img_name1, "img_name2": img_name2, "cnt_0": cnt_0,
