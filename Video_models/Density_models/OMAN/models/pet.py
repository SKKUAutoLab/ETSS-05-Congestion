...
from .transformer import window_partition, build_encoder, build_decoder
...
class PET(nn.Module):
    def __init__(self, backbone, num_classes, args=None):
        ...
        hidden_dim = args.hidden_dim
        self.input_proj = nn.ModuleList([nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1), nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1)])
        ...
        self.quadtree_splitter = nn.Sequential(nn.AvgPool2d((context_h, context_w), stride=(context_h ,context_w)),
    ...
    def forward(self, samples: NestedTensor, inputs, labels, **kwargs):
        features, pos = self.backbone(samples) # pos: [1, 256, 160, 256], [1, 256, 80, 128]
        dense_input_embed = self.pos_embed(samples) # [1, 256, 1280, 2048]
        ...
        if 'train' in kwargs:
            ...
            out = self.test_forward(samples, features, pos, **kwargs) # [30, 2]
        return out
    ...
    def train_forward(self, samples, inputs, labels, features, pos, **kwargs):
        ...
        pet_loss = self.compute_loss(pet_outputs, criterion, targets, epoch)
        losses = {}
    ...
    def test_forward(self, samples, features, pos, **kwargs):
        ...
        if self.args.type_dataset == 'HT21':
            return div_out, features
        elif self.args.type_dataset == 'SENSE':
            ...
            points = duplicate(points, samples.tensors.squeeze(0)) # [30, 2]
            return points, features
        else:
            print('This dataset does not exist')
            raise NotImplementedError

    def forward_single_image(self, x, pts, feature, absolute=False, pre_z=None): # [1, 3, 1080, 1920], [30, 2], [1, 256, 160, 256]
        z_lists = []
        pos_list = []
        _, _, h, w = x.shape
...
def duplicate(outputs_points, frame): # [30, 2], [3, 1280, 2048]
    del_list = []
    del_pts = []
    ...
    outputs_points = np.delete(outputs_points, del_list, axis=0) # [30, 2]
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_points, **kwargs):
...
def build_pet(args):
    ...
    model = PET(backbone, num_classes=num_classes, args=args)
    matcher = build_matcher(args)
    weight_dict = {'loss_ce': args.ce_loss_coef, 'loss_points': args.point_loss_coef}
    losses = ['labels', 'points']
    criterion = SetCriterion(num_classes, matcher=matcher, weight_dict=weight_dict, eos_coef=args.eos_coef, losses=losses, args=args)
    criterion.cuda()
