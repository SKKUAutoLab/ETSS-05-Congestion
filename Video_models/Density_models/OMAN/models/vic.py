...
class VIC_Model(nn.Module):
    def __init__(self, backbone, stride=16, num_feature_levels=1, num_channels=768, proj_channel=256, hidden_dim=256, freeze_backbone=False) -> None:
        super().__init__()
        ...
        self.input_fuse = nn.Sequential(nn.Conv2d(hidden_dim * num_feature_levels, hidden_dim, kernel_size=1, stride=1, padding=0), nn.GroupNorm(32, hidden_dim), nn.GELU())
