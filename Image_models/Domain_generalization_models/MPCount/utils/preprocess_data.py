    ...
    for phase in ['Train', 'Test']:
        ...
        if phase == 'Train':
            ...
            for sub_phase in sub_phase_list:
                ...
                if not os.path.exists(sub_save_dir):
                    os.makedirs(sub_save_dir)
                with open('configs/ucf_{}.txt'.format(sub_phase)) as f:
                    for i in f:
...
def run_sta(origin_dir, save_dir, min_size, max_size):
    ...
    for phase in ['train', 'test']:
        ...
        if phase == 'train':
            ...
            with open(f'configs/{part}_train.txt') as f:
                train_list = f.readlines()
            with open(f'configs/{part}_val.txt') as f:
            ...
            for sub_phase, im_list in zip(sub_phase_list, [train_list, val_list]):
                ...
                for im_path in tqdm(im_list):
                    name = os.path.basename(im_path)
                    im, points = generate_data_sta(im_path, min_size, max_size) # [685, 1024, 3], [321, 2]
                    im_save_path = os.path.join(sub_save_dir, name)
                    im.save(im_save_path)
                    gd_save_path = im_save_path.replace('jpg', 'npy')
                    np.save(gd_save_path, points)
...
def run_fdst(origin_dir, save_dir, min_size, max_size):
    for phase in ['train', 'test']:
        ...
        if phase == 'train':
            ...
            im_list = glob(os.path.join(im_path, '*jpg'))
            train_list = [1,2,6,7,11,12,16,17,21,22,26,27,31,32,36,37,41,42,46,47,51,52,56,57,61,62,66,67,71,72,76,77,81,82,86,87,91,92,96,97]
            for sub_phase in sub_phase_list:
                sub_save_dir = os.path.join(save_dir, sub_phase)
                if not os.path.exists(sub_save_dir):
                    os.makedirs(sub_save_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='data/sta', type=str)
    parser.add_argument('--min_size', default=512, type=int)
    parser.add_argument('--max_size', default=2048, type=int)
    parser.add_argument('--type_dataset', default='sta', type=str, choices=['jhu', 'qnrf', 'smartcity', 'sta', 'stb', 'cc50', 'fdst', 'vidcrowd', 'worldexpo', 'nwpu', 'mall', 'ucsd'])
    ...
    if args.type_dataset == 'jhu':
        ...
        run_qnrf(args.input_dir, args.output_dir, args.min_size, args.max_size)
    elif args.type_dataset == 'smartcity':
        run_smartcity(args.input_dir, args.output_dir, args.min_size, args.max_size)
    elif args.type_dataset == 'sta' or args.type_dataset == 'stb':
        run_sta(args.input_dir, args.output_dir, args.min_size, args.max_size)
    elif args.type_dataset == 'cc50':
        run_cc50(args.input_dir, args.output_dir, args.min_size, args.max_size)
    elif args.type_dataset == 'fdst':
        run_fdst(args.input_dir, args.output_dir, args.min_size, args.max_size)
    elif args.type_dataset == 'vidcrowd':
        run_vidcrowd(args.input_dir, args.output_dir, args.min_size, args.max_size)
    elif args.type_dataset == 'nwpu':
        run_nwpu(args.input_dir, args.output_dir, args.min_size, args.max_size)
    elif args.type_dataset == 'mall':
        run_mall(args.input_dir, args.output_dir, args.min_size, args.max_size)
    elif args.type_dataset == 'ucsd':
        run_ucsd(args.input_dir, args.output_dir, args.min_size, args.max_size)
    else:
