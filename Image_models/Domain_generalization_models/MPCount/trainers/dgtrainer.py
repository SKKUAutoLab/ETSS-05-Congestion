...
class DGTrainer(Trainer):
    ...
    def predict(self, model, img): # [1, 3, 768, 1024]
        ...
        if h >= ps or w >= ps:
            ...
            pred_dmap = model(img) if self.mode == 'base' else model(img)[0] # [1, 1, 768, 1024]
    ...
    def train_step(self, model, loss, optimizer, batch, epoch):
        ...
        if self.mode == 'simple':
            ...
            optimizer.zero_grad()
            dmaps1 = model(imgs1) # [16, 1, 320, 320], [16, 1, 20, 20]
            dmaps2 = model(imgs2) # [16, 1, 320, 320], [16, 1, 20, 20]
            ...
            optimizer.zero_grad()
            dmaps1, cmaps1 = model(imgs1, gt_cmaps) # [16, 1, 320, 320], [16, 1, 20, 20]
            dmaps2, cmaps2 = model(imgs2, gt_cmaps) # [16, 1, 320, 320], [16, 1, 20, 20]
            loss_den = self.compute_count_loss(loss, dmaps1, gt_datas) + self.compute_count_loss(loss, dmaps2, gt_datas)
            ...
            optimizer.zero_grad()
            gts = gt_datas[1].to(self.device) # [16, 1, 320, 320]
