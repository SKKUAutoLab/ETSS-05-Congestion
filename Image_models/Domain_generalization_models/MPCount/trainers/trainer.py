...
class Trainer(object):
    ...
    def load_ckpt(self, model, path):
        if path is not None:
            self.log('Load ckpt from: {}'.format(path))
    ...
    def train_epoch(self, model, loss, train_dataloader, val_dataloader, optimizer, scheduler, epoch, best_criterion, best_epoch):
        ...
        self.log(f'Epoch: {epoch}, Validation criterion: {current_criterion:.4f}', end=' ')
        for k, v in additional_meter.avg.items():
            self.log(f'{k}: {v:.4f}', end=' ')
        self.log(f'Best score: {best_criterion:.4f}, Total time: {time.time() - start_time:.4f}')
        # save checkpoint
        if epoch > 0:
