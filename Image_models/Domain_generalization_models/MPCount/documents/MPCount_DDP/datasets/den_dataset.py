...
class DensityMapDataset(BaseDataset):
    ...
    def _train_transform(self, img, gt, dmap): # [512, 934, 3], [643, 2], [512, 934]
        w, h = img.size
        assert len(gt) >= 0
        ...
        if random.random() > 0.5:
            ...
            if len(gt) > 0:
                gt[:, 0] = w - gt[:, 0]
        img = self.transform(img) # [3, 320, 320]
        gt = torch.from_numpy(gt.copy()).float() # [55, 2]
