...
class DGModel_final(nn.Module):
    def __init__(self, pretrained=True, mem_size=1024, mem_dim=256, cls_thrs=0.5, err_thrs=0.5, den_dropout=0.5,
                 cls_dropout=0.3, has_err_loss=False, deterministic=True):
        super().__init__()
        # DGModel_base params
    ...
    def forward_test(self, x, c_gt=None):  # [1, 3, 768, 1024], None
    ...
    def forward(self, img1, img2=None, c_gt=None, train=True):
        if train:
            dc1, dc2, c1, c2, c_err, loss_con, loss_err = self.forward_train(img1, img2, c_gt)
            return dc1, dc2, c1, c2, c_err, loss_con, loss_err
        else:
            dc, c = self.forward_test(img1, c_gt)
            return dc, c
...
if __name__ == '__main__':
    ...
    # train
    model.train()
    img1 = torch.rand(4, 3, 320, 320).cuda()
    img2 = torch.rand(4, 3, 320, 320).cuda()
    c_gt = torch.rand(4, 1, 20, 20).cuda()
    dc1, dc2, c1, c2, c_err, loss_con, loss_err = model(img1=img1, img2=img2, c_gt=c_gt, train=True)  # [4, 1, 320, 320], [4, 1, 320, 320], [4, 1, 20, 20], [4, 1, 20, 20], [4, 1, 320, 320], [1], 0
    # test
    model.eval()
    img = torch.rand(1, 3, 320, 320).cuda()
    dc, c = model(img1=img, img2=None, c_gt=None, train=False)  # [1, 1, 320, 320], [1, 1, 20, 20]