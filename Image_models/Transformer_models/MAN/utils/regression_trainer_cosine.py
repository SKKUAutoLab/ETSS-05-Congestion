...
class RegTrainer(Trainer):
    def setup(self):
        ...
        elif args.type_dataset == 'qnrf' or args.type_dataset == 'jhu':
    ...
    def train_eopch(self):
        ...
        logging.info('Epoch {} Train, Loss: {:.2f}, MAE: {:.2f}, MSE: {:.2f}, Cost {:.1f} sec'
                     .format(self.epoch, epoch_loss.get_avg(), epoch_mae.get_avg(), np.sqrt(epoch_mse.get_avg()), time.time()-epoch_start))
    ...
    def val_epoch(self):
        ...
        model_state_dic = self.model.state_dict()
        logging.info("Best MAE: {:.2f}, best MSE: {:.2f}".format(self.best_mae, self.best_mse))
        if (2.0 * mae + mse) < (2.0 * self.best_mae + self.best_mse):
            self.best_mae = mae
            self.best_mse = mse
            logging.info("Save best MAE {:.2f}, best MSE {:.2f} at epoch {}".format(self.best_mae, self.best_mse, self.epoch))
            if self.save_all:
