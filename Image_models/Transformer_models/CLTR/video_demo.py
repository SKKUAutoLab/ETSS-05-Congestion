...
def main(args):
    ...
    width = 1024
    height = 768
    out = cv2.VideoWriter('output.avi', fourcc, 30, (width * 2, height * 2))
    # calculate FPS
    ...
    while True:
        ...
        image = tensor_transform(frame) # [3, 768, 1024]
        image = img_transform(image) # [3, 768, 1024]
        width, height = image.shape[2], image.shape[1]
        num_w = int(width / 256)
        num_h = int(height / 256)
        image = image.view(3, num_h, 256, width).view(3, num_h, 256, num_w, 256) # [3, 3, 256, 4, 256]
        image = image.permute(0, 1, 3, 2, 4).contiguous().view(3, num_w * num_h, 256, 256).permute(1, 0, 2, 3) # [12, 3, 256, 256]
        with torch.no_grad():
            image = image.cuda()
            outputs = model(image)
            out_logits, out_point = outputs['pred_logits'], outputs['pred_points'] # [12, 500, 2], [12, 500, 3]
            prob = out_logits.sigmoid()
            topk_values, topk_indexes = torch.topk(prob.view(out_logits.shape[0], -1), args['num_queries'], dim=1) # [12, 500], [12, 500]
            topk_points = topk_indexes // out_logits.shape[2]
            out_point = torch.gather(out_point, 1, topk_points.unsqueeze(-1).repeat(1, 1, 2)) # [12, 500, 2]
            out_point = out_point * 256
            value_points = torch.cat([topk_values.unsqueeze(2), out_point], 2) # [12, 500, 3]
            crop_size = 256
            kpoint_map, density_map, frame, count = show_map(value_points, frame, width, height, crop_size, num_h, num_w)
            res1 = np.hstack((ori_frame, kpoint_map)) # [768, 2048, 3]
            res2 = np.hstack((density_map, frame)) # [768, 2048, 3]
            res = np.vstack((res1, res2)) # [1536, 2048, 3]
            # calculate FPS
            frame_count += 1
