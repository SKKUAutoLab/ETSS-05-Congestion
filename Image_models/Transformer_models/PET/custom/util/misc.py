    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                ...
                if torch.cuda.is_available():
                    print(log_msg.format(i, len(iterable), eta=eta_string, meters=meters, time=str(iter_time), data=str(data_time), memory=torch.cuda.max_memory_allocated() / MB))
                else:
                    print(log_msg.format(i, len(iterable), eta=eta_string, meters=meters, time=str(iter_time), data=str(data_time)))
            i += 1
            end = time.time()
