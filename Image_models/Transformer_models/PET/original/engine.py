...
def visualization(samples, targets, pred, vis_dir, split_map=None):
    ...
    for idx in range(images.shape[0]):
        ...
        size = 2
        for t in gts[idx]:
            sample_vis = cv2.circle(sample_vis, (int(t[1]), int(t[0])), size, (0, 0, 255), -1) # red GT points
        for p in pred[idx]:
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer, epoch: int, max_norm: float = 0):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        samples = samples.to(torch.device('cuda')) # [8, 3, 256, 256]
        targets = [{k: v.cuda() for k, v in t.items()} for t in targets]
        gt_points = [target['points'] for target in targets] # [93, 2]
...
@torch.no_grad()
def evaluate(model, data_loader, vis_dir=None):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    if vis_dir is not None:
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        outputs_scores = torch.nn.functional.softmax(outputs['pred_logits'], -1)[:, :, 1][0] # [115]
        outputs_points = outputs['pred_points'][0] # [115, 2]
        outputs_offsets = outputs['pred_offsets'][0] # [115, 2]
