...
class CrowdCounter(nn.Module):
    ...
    def forward(self, data, num_class, iter=0, mode='train'):
        rgb, nir = data['rgb'], data['nir'] # [4, 3, 512, 512], [4, 3, 512, 512]
        nir = nir[:, 0, :, :].unsqueeze(1)
        if self.net_name == 'MCC':
            pred_map = self.CCN(rgb, nir) # [4, 6, 64, 64]
        else:        
            pred_map = self.CCN(rgb)
        outputs = {'pred_map': pred_map}
        if mode == 'train':
            gt_map = data['gt_map'][:, :num_class, :, :]
            if cfg.LOSS_FUNCTION == 'Cos_loss':
                gauss_map = self.multi_class_gauss_map_generate(gt_map)
                self.loss_fn = self.build_cos_loss(pred_map, gt_map, iter, alpha=self.alpha, beta=1)
                outputs['gauss_map'] = gauss_map
                return outputs
            elif cfg.LOSS_FUNCTION == 'Mse_loss':
                gauss_map = self.multi_class_gauss_map_generate(gt_map)
                self.loss_fn = self.build_mse_loss(pred_map, gauss_map, iter)
                outputs['gauss_map'] = gauss_map # [4, 6, 64, 64]
                return outputs
            elif cfg.LOSS_FUNCTION == 'Mix_loss':
                gauss_map = self.multi_class_gauss_map_generate(gt_map)
    ...
    def build_cos_loss(self, pred_map, gauss_map, iter, alpha, beta):
        mse_loss = self.loss_mse_fn(pred_map, gauss_map)  
        c_loss = self.loss_cossim_fn(pred_map).cuda()
        loss = alpha * c_loss + beta * mse_loss
        if (iter + 1) % 20 == 0:
            print('MSE loss: {:.4f}, Cos loss: {:.4f}'.format(mse_loss, c_loss))
        return loss

    def build_mse_loss(self, pred_map, gauss_map, iter):
        loss = self.loss_mse_fn(pred_map, gauss_map)
        if (iter + 1) % 20 == 0:
            print('MSE: loss: {:.4f}'.format(loss))
        return loss
    ...
    def build_mix_loss(self, pred_map, gauss_map, iter):  
        ...
        if (iter + 1) % 20 == 0:
            print('MSE loss: {:.4f}, L1 loss: {:.4f}'.format(mse_loss, l1_loss))
        return loss
    ...
    def build_mask_loss(self, pred_map, gauss_map, mask_map, pred_mask_maps, iter):  
        ...
        if (iter + 1) % 20 == 0:
            print('MSE loss: {:.4f}, Seg loss: {:.4f}'.format(mse_loss, seg_loss))
        return loss
    
    def multi_class_gauss_map_generate(self, gt_map): # [4, 6, 64, 64]
        ...
        gauss_map = torch.stack(class_gauss_maps,1) # [4, 6, 64, 64]
        return gauss_map