...
class RandomCrop(object):
    ...
    def __call__(self, rgb, nir, gt_map, dst_size=None):
        ...
        return rgb.crop((x1, y1, x1 + tw, y1 + th)), nir.crop((x1, y1, x1 + tw, y1 + th)), gt_map[y1:y1 + th, x1:x1 + tw, :]
...
class ScaleByRateWithMin(object):
    ...
    def __call__(self, rgb, nir, gt_map):
        ...
        for i in range(class_num):
            ...
            assert new_gt_map[:,:,i].sum() == gt_map[:,:,i].sum(), "new_gt_map:{}, gt_map:{}".format(new_gt_map[:,:,i].sum(), gt_map[:,:,i].sum())
