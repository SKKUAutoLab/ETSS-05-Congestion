...
class Crowd_qnrf(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        gd_path = os.path.join(self.root_path, '{}.npy'.format(name))
    ...
    def __getitem__(self, item):
        ...
        if self.method == 'train':
            ...
            if st_size < self.c_size:
                ...
                img = img.resize((wd, ht), Image.BICUBIC)
            img = self.trans(img)
            gt_count = np.float16(len(keypoints))
...
if __name__ == '__main__':
    ...
    datasets = {x: Crowd_nwpu(os.path.join('/home/hdd/dataset_xzw/NWPU-Crowd-576x2048', x), 512, 8, x) for x in ['train', 'val']}
    ...
    for inputs, count, name in dataloaders['val']:
        ...
        # img = inputs[0].squeeze(0).transpose(0,2).transpose(0,1)
        # vis_img = img.numpy()
        # vis_img = (vis_img - vis_img.min()) / (vis_img.max() - vis_img.min() + 1e-5)
        # vis_img = (vis_img * 255).astype(np.uint8)
        # vis_img = cv2.applyColorMap(vis_img, cv2.COLORMAP_JET)
        # cv2.imwrite('/home/xuzhiwen/home/xuzhiwen/dataset_wfs/TransDensityNet/vis/img.png', vis_img)
        print('*************')
        exit(-1)