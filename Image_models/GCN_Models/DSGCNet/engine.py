...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer, Dens_Loss, max_norm: float = 0):
    model.train()
    criterion.train()
    metric_logger = utils.MetricLogger(delimiter=" ")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    for samples, targets, gt_dmap in data_loader:
        samples = samples.cuda() # [32, 3, 128, 128]
        gt_dmap = torch.stack(gt_dmap) # [32, 1, 16, 16]
        gt_dmap = gt_dmap.cuda()
        targets = [{k: v.cuda() for k, v in t.items()} for t in targets]
        outputs = model(samples) # pred_logits: [32, 1024, 2], pred_points: [32, 1024, 2], density_out: [32, 1, 16, 16]
        loss_dict = criterion(outputs, targets)
        weight_dict = criterion.weight_dict
        losses = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if k in weight_dict)
        et_dmap = outputs['density_out'] # [32, 1, 16, 16]
...
@torch.no_grad()
def evaluate_crowd_no_overlap(model, data_loader):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter=" ")
    ...
    for samples, targets in data_loader:
        ...
        outputs = model(samples) # [1, 49152, 2], [1, 49152, 2], [1, 1, 96, 128]
        ...
        et_dmap = outputs['density_out'] # [1, 1, 96, 128]
