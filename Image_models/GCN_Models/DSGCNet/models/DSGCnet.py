...
class Density_pred(nn.Module):
    def __init__(self):
        ...
        self.v1 = nn.Sequential(nn.Conv2d(256, 256, 3, padding=1, dilation=1), nn.BatchNorm2d(256), nn.ReLU(inplace=True))
        self.v2 = nn.Sequential(nn.Conv2d(256, 256, 3, padding=1, dilation=1), nn.BatchNorm2d(256), nn.ReLU(inplace=True))
        self.v3 = nn.Sequential(nn.Conv2d(256, 256, 3, padding=1, dilation=1), nn.BatchNorm2d(256), nn.ReLU(inplace=True))
        self.conv_layers = nn.Sequential(nn.Conv2d(256, 128, kernel_size=3, padding=1), nn.BatchNorm2d(128), nn.ReLU(inplace=True),
                                         nn.Conv2d(128, 64, kernel_size=3, padding=1), nn.BatchNorm2d(64), nn.ReLU(inplace=True),
                                         nn.Conv2d(64, 1, kernel_size=1), nn.ReLU(inplace=True))

    def forward(self, x): # [32, 256, 16, 16]
        x = self.v1(x)
        x = self.v2(x)
        x = self.v3(x)
        y = self.conv_layers(x) # [32, 1, 16, 16]
        return y
...
class SPD(nn.Module):
    ...
    def forward(self, x): # [32, 256, 32, 32]
        return torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]], 1) # [32, 1024, 16, 16]

class Decoder_SPD_PAFPN(nn.Module):
    def __init__(self, C3_size, C4_size, C5_size, feature_size=256):
        super(Decoder_SPD_PAFPN, self).__init__()
        self.P5_1 = nn.Sequential(nn.Conv2d(C5_size, feature_size, kernel_size=1, stride=1, padding=0), nn.BatchNorm2d(feature_size), nn.ReLU(inplace=True))
        self.P5_upsampled = nn.Upsample(scale_factor=2, mode='nearest')
        self.P5_2 = nn.Sequential(nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1), nn.BatchNorm2d(feature_size), nn.ReLU(inplace=True))
        self.P4_1 = nn.Sequential(nn.Conv2d(C4_size, feature_size, kernel_size=1, stride=1, padding=0), nn.BatchNorm2d(feature_size), nn.ReLU(inplace=True))
        self.P4_upsampled = nn.Upsample(scale_factor=2, mode='nearest')
        self.P4_2 = nn.Sequential(nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1), nn.BatchNorm2d(feature_size), nn.ReLU(inplace=True))
        self.P3_1 = nn.Sequential(nn.Conv2d(C3_size, feature_size, kernel_size=1, stride=1, padding=0), nn.BatchNorm2d(feature_size), nn.ReLU(inplace=True))
        self.P3_2 = nn.Sequential(nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1), nn.BatchNorm2d(feature_size), nn.ReLU(inplace=True))
        self.P3_downsampled = nn.Sequential(SPD(), nn.Conv2d(4 * feature_size, feature_size, kernel_size=1), nn.BatchNorm2d(feature_size), nn.ReLU(inplace=True))
        self.P4_downsampled = nn.Sequential(SPD(), nn.Conv2d(4 * feature_size, feature_size, kernel_size=1), nn.BatchNorm2d(feature_size), nn.ReLU(inplace=True))
        self.fusion = nn.Sequential(nn.Conv2d(3 * feature_size, feature_size, kernel_size=1), nn.BatchNorm2d(feature_size), nn.ReLU(inplace=True))

    def forward(self, inputs):
        C3, C4, C5 = inputs # [32, 256, 32, 32], [32, 512, 16, 16], [32, 512, 8, 8]
        P5_x = self.P5_1(C5)
        ...
        fuse = self.fusion(fuse) # [32, 256, 16, 16]
        return fuse
...
class DSGCnet(nn.Module):
    def __init__(self, backbone, row=2, line=2):
        ...
        self.alpha = nn.Parameter(torch.tensor([1.0, 1.0], dtype=torch.float32, requires_grad=True))
    ...
    def forward(self, samples: NestedTensor): # [32, 3, 128, 128]
        ...
        out = {'pred_logits': output_class, 'pred_points': output_coord, 'density_out': density} # [32, 1024, 2], [32, 1024, 2], [32, 1, 16, 16]
        return out
