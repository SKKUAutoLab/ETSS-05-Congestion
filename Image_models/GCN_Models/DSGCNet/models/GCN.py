...
class DensityGraphBuilder:
    ...
    def build_batch_graph(self, density_maps): # [32, 1, 16, 16]
        ...
        edge_index, _ = add_self_loops(edge_index, num_nodes=num_nodes_total) # [2, 40960]
...
class FeatureGraphBuilder:
    ...
    def build_batch_graph(self, feature_maps): # [32, 256, 16, 16]
...
class GCNModel(nn.Module):
    ...
    def forward(self, tensor, edge_index): # [8192, 256], [2, 40960]
        x = self.conv1(tensor, edge_index)
        x = F.relu(x)
        x = F.dropout(x, p=0.5)
        x = self.conv2(x, edge_index)
        x = F.relu(x)
        x = F.dropout(x, p=0.5) # [8192, 256]
        return x
...
class DensityGCNProcessor(nn.Module):
    ...
    def forward(self, density_maps, feature_maps): # [32, 1, 16, 16], [32, 256, 16, 16]
        ...
        Density_GCN_map = out.view(B, H, W, in_channels).permute(0, 3, 1, 2).contiguous() # [32, 256, 16, 16]
...
class FeatureGCNProcessor(nn.Module):
    ...
    def forward(self, feature_maps): # [32, 256, 16, 16]
        B, C, H, W = feature_maps.shape
        ...
        out = self.gcn(node_features, edge_index)
        Feature_GCN_map = out.view(B,H,W,C).permute(0, 3, 1, 2).contiguous() # [32, 256, 16, 16]
