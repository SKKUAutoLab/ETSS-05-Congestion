...
def vis(samples, targets, pred, vis_dir, des=None):
    ...
    for idx in range(samples.shape[0]):
        ...
        size = 2
        for t in gts[idx]:
            sample_gt = cv2.circle(sample_gt, (int(t[0]), int(t[1])), size, (0, 255, 0), -1) # red GT points
        for p in pred[idx]:
            sample_pred = cv2.circle(sample_pred, (int(p[0]), int(p[1])), size, (0, 0, 255), -1) # green prediction points
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer, max_norm: float = 0):
    model.train()
    criterion.train()
    ...
    for samples, targets in data_loader:
        ...
        outputs = model(samples) # [32, 1024, 2], [32, 1024, 2]
