    ...
    def validate(self, model, loader):
        ...
        with torch.no_grad():
            for i, data in enumerate(loader):
                ...
                if 'det' in model.args.model:
                    self.global_err += 0
                    acc, prec, recall = model.get_acc_prec_recall(model.output.detach(), model.target)
                    self.acc += acc
                    self.prec += prec
                    self.recall += recall
                elif 'direct' not in model.args.model and 'sep' not in model.args.model and 'localize' not in model.args.model:
                    ...
                    self.global_err += abs(model.dmap.detach().sum() - model.target.detach().sum())
        self.p = self.p / len(loader) / model.args.output_cn
        ...
        if 'direct' not in model.args.model and 'sep' not in model.args.model and 'localize' not in model.args.model:
            print('#' + self.suffix, end=': ')
            print('Total:  %.4f, Acc: %.2f' % (self.global_err / len(loader), (self.acc / len(loader))))
            print('#' + self.suffix, end=': ')
            print('Precision and Recall', end=', ')
            print(self.prec / len(loader), self.recall / len(loader))
        elif 'localize' in model.args.model:
