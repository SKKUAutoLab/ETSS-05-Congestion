...
class ContextualModule(nn.Module):
    ...
    def forward(self, feats): # [26, 512, 48, 64]
        ...
        bottle = self.bottleneck(torch.cat(overall_features, 1)) # [26, 512, 48, 64]
...
class CANNet(nn.Module):
    ...
    def forward(self, x): # [26, 3, 384, 512]
        x = self.frontend(x)
        x = self.context(x)
        x = self.backend(x)
        x = self.output_layer(x) # [26, 1, 48, 64]
        return x
