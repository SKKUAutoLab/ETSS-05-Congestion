...
def main(args):
    ...
    try:
        ...
        np.save(os.path.join(args.output_dir, 'nwpu_train_2048.npy'), train_img_list)
        val_img_list = []
        for i in range(len(val_list)):
            fname = val_list[i].split(' ')[0] + '.jpg'
            val_img_list.append(root + fname)
        np.save(os.path.join(args.output_dir, 'nwpu_val_2048.npy'), val_img_list)
        test_img_list = []
        root = root.replace('images_2048','test_data_2048')
        for i in range(len(test_list)):
            fname = test_list[i].split(' ')[0] + '.jpg'
            test_img_list.append(root + fname)
        np.save(os.path.join(args.output_dir, 'nwpu_test_2048.npy'), test_img_list)
    except:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nwpu_dir', type=str, default='datasets/NWPU_localization')
