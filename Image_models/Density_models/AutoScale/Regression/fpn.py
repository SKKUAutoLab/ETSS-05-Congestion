...
class AutoScale(nn.Module):
    def __init__(self, load_weights=False):
        ...
        self.cd2 = nn.Sequential(nn.Conv2d(128,32,3,padding=1), nn.ReLU(inplace=True))
        self.cd3 = nn.Sequential(nn.Conv2d(256,32,3,padding=1), nn.ReLU(inplace=True))
        self.cd4 = nn.Sequential(nn.Conv2d(512,32,3,padding=1), nn.ReLU(inplace=True))
        self.cd5 = nn.Sequential(nn.Conv2d(512,32,3,padding=1), nn.ReLU(inplace=True))
        self.rd5 = nn.Sequential(nn.Conv2d(32, 8, 1), nn.ReLU(inplace=True))
        self.rd4 = nn.Sequential(nn.Conv2d(40, 8, 1), nn.ReLU(inplace=True))
        self.rd3 = nn.Sequential(nn.Conv2d(40, 8, 1), nn.ReLU(inplace=True))
