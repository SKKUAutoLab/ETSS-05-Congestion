...
def createTrainData(datasetname, Dataset, cfg_data):
    ...
    train_set = Dataset(datasetname, 'train', main_transform=main_transform, img_transform=img_transform, mask_transform=mask_transform, list_file=list_file)
...
def createValData(datasetname, Dataset, cfg_data):
    ...
    img_transform = standard_transforms.Compose([standard_transforms.ToTensor(), standard_transforms.Normalize(*cfg_data.MEAN_STD)])
    mask_transform = standard_transforms.Compose([standard_transforms.ToTensor()])
    val_set = Dataset(datasetname, 'val', img_transform=img_transform, mask_transform=mask_transform, list_file=list_file)
    if datasetname in ['SHHA', 'SHHB', 'QNRF', 'JHU', 'NWPU']:
