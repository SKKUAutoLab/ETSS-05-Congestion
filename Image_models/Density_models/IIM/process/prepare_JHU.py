...
def resize_images(mode, args):
    imgs_path = os.path.join(args.input_dir, mode, 'images')
    gt_path = os.path.join(args.input_dir, mode, 'gt')
...
def generate_masks(args):
    ...
    for idx, img_path in enumerate(file_list):
        ...
        dst_vis_path = os.path.join(args.output_dir, 'box_vis')
        if not os.path.exists(dst_vis_path):
            os.makedirs(dst_vis_path)
        plt.savefig(os.path.join(dst_vis_path, img_id+'.jpg'), bbox_inches='tight', pad_inches=0, dpi=300)
        plt.close()
...
def loc_gt_make(mode='test', args=None):
    ...
    for idx, img_id in enumerate(img_ids):
        json_path = os.path.join(args.output_dir, 'jsons', img_id + '.json')
        Box_Info = []
...
def JHU_list_make(mode, args):
    path = os.path.join(args.input_dir, mode, 'images')
    for filname in os.listdir(path):
        filname = filname.split('.')[0]
        with open(args.output_dir + '/' + mode + '.txt','a') as f:
            f.write(filname + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='data/JHU')
    ...
    resize_images('train', args)
    resize_images('val', args)
    resize_images('test', args)
    generate_masks(args)
    JHU_list_make('train', args)
    JHU_list_make('val', args)
    JHU_list_make('test', args)
