...
class Crowd(Dataset):
    def __init__(self, root, crop_size, d_ratio, method='train'):
        ...
        if method not in ['train', 'val']:
            print('This type of method does not exist')
            raise NotImplementedError
        self.method = method
        self.transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize([0.452016860247, 0.447249650955, 0.431981861591], [0.23242045939, 0.224925786257, 0.221840232611])])
    ...
    def __getitem__(self, item):
        ...
        img = Image.open(im_path).convert('RGB') # [640, 480, 3]
        keypoints = np.load(gd_path) # [77, 2]
        if self.method == 'train':
            den_path = os.path.join(self.root, 'gt_den', '{}.h5'.format(name))
            den_map = h5py.File(den_path, 'r')['density_map'] # [640, 480]
    ...
    def train_transform(self, img, d_map): # [640, 480, 3], [640, 480]
        wd, ht = img.size
        ...
        return self.transform(img), torch.from_numpy(d_map.copy()).float().unsqueeze(0), torch.from_numpy(b_map.copy()).float().unsqueeze(0)