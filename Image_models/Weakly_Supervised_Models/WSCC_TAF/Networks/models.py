...
def base_patch16_384_mamba(pretrained=False, mode = 0, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.load('Networks/vim_t_midclstok_76p1acc.pth', map_location='cpu')
        state_dict = checkpoint["model"]
        # interpolate pos_embed (14x14 grid + middle cls token) to this model's grid
        if state_dict["pos_embed"].shape != model.pos_embed.shape:
            pos_embed = state_dict["pos_embed"]
            embed_dim = pos_embed.shape[-1]
            num_patches_old = pos_embed.shape[1] - 1
            cls_pos_old = num_patches_old // 2
            cls_pe = pos_embed[:, cls_pos_old:cls_pos_old + 1, :]
            patch_pe = torch.cat([pos_embed[:, :cls_pos_old, :], pos_embed[:, cls_pos_old + 1:, :]], dim=1)
            old_size = int(num_patches_old ** 0.5)
            new_grid = model.patch_embed.grid_size
            patch_pe = patch_pe.reshape(1, old_size, old_size, embed_dim).permute(0, 3, 1, 2)
            patch_pe = F.interpolate(patch_pe, size=new_grid, mode='bicubic', align_corners=False)
            patch_pe = patch_pe.permute(0, 2, 3, 1).reshape(1, new_grid[0] * new_grid[1], embed_dim)
            cls_pos_new = (new_grid[0] * new_grid[1]) // 2
            state_dict["pos_embed"] = torch.cat([patch_pe[:, :cls_pos_new, :], cls_pe, patch_pe[:, cls_pos_new:, :]], dim=1)
