...
from .channel_block import ChannelBlock
from .layers.coordatten import CoordAtt
...
def create_block(
    ...
    mixer_cls = partial(Mamba, layer_idx=layer_idx, bimamba_type=bimamba_type, if_divide_out=if_devide_out, init_layer_scale=init_layer_scale, **ssm_cfg, **factory_kwargs)
    norm_cls = partial(
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        if if_cls_token:
            if use_double_cls_token:
                ...
                self.num_tokens = 2
            else:#jump into this command
                self.cls_token = nn.Parameter(torch.zeros(1, 1, self.embed_dim)) #run this line
                # self.num_tokens = 1
        ...
        if if_abs_pos_embed:
            trunc_normal_(self.pos_embed, std=.02)# run
        if if_cls_token:
            if use_double_cls_token:
                trunc_normal_(self.cls_token_head, std=.02)
                trunc_normal_(self.cls_token_tail, std=.02)
            else:
                trunc_normal_(self.cls_token, std=.02)#run
        ...
        if self.mode == 3:
            self.post_conv = nn.Sequential(
                Res_SE1D(192, 192, stride=1), # B, 1024, 284
            )
        elif self.mode == 2:
            self.post_conv = nn.Sequential(
                Res_SE1D(192, 192, stride=1, use_se=False), # B, 1024, 284
            )
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.if_cls_token: #run this line
            if self.use_double_cls_token:
                ...
                if self.use_middle_cls_token:#run_this_line
        ...
        # mamba impl
        residual = None
        hidden_states = x#torch.Size([10, 577, 192])
        if not self.if_bidirectional:
            for layer in self.layers:

                if if_flip_img_sequences and self.if_rope:#no
                ...
                hidden_states, residual = layer(#run this torch.Size([10, 577, 192])
