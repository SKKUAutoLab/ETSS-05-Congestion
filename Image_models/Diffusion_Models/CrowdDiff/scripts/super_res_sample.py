...
def main():
    ...
    for _ in os.listdir(args.data_dir):
        ...
        if model_kwargs["low_res"].size(0) == 0:
            print(f"Skipping {data_parameter.name}: no valid crops after crowd_count filtering")
            continue
        
        # model_kwargs['low_res'] = model_kwargs['low_res']#[:2]
        ...
        while data_parameter.resample:
            ...
            if len(count) > 0:
                ...
                if avg_mae is None:
                    avg_mae = np.expand_dims(abs(count-gt_count),axis=0)
                else:
                    avg_mae = np.concatenate([avg_mae, np.expand_dims(abs(count-gt_count),axis=0)], axis=0)
                count = [_ for _ in count]
                gt_count = [gt_count]
                gt_count.extend(count)
