...
if [ $VOLUME_ESTIMATION -eq 1 ]; then
    ...
    if [ -f "$path_3dgs" ]; then
        ...
        ns-train counting-splatfacto --data $folder_masked --viewer.quit-on-train-completion True --experiment-name $base_name --timestamp ${exp_name} \
                                     --pipeline.model.background_color "random" --max-num-iterations 8000 nerfstudio-data --auto-scale-poses False \
                                     --scale-factor 1 --center_method none --orientation_method none --scene_scale 1.0
    fi
