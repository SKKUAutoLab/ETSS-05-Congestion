...
class GroundedSAM(BaseImageSegmentation):
    ...
    def load_model(self, model_config_path, model_checkpoint_path, device):
        self.grounding_dino_model = Model(model_config_path=self.model_config_path, model_checkpoint_path=self.model_checkpoint_path)
    ...
    def run(self, image_path: Union[Path, str], text_prompt: Optional[str], output_filename, output_dir: Union[Path, str], box_threshold: float = 0.35, text_threshold: float = 0.35, flag_segmentation_image_debug=False):
        ...
        detections = self.grounding_dino_model.predict_with_classes(image=image, classes=CLASSES, box_threshold=BOX_THRESHOLD, text_threshold=TEXT_THRESHOLD)
        box_annotator = sv.BoxAnnotator()
        labels = [f"{CLASSES[class_id]} {confidence:0.2f}" for confidence, class_id in zip(list(detections.confidence), list(detections.class_id))]
        ...
        detections.mask = segment(sam_predictor=self.sam_predictor, image=cv2.cvtColor(image, cv2.COLOR_BGR2RGB), xyxy=detections.xyxy)
...
class SegmentImages(object):
    def __init__(self, model: Literal['grounded_sam'] = 'grounded_sam', device: Literal['cpu', 'cuda'] = 'cpu', debug: bool = False):
    ...
    def run(self, image_path, text_prompt, output_filename, output_dir, box_threshold: float, text_threshold: float, flag_segmentation_image_debug: bool):
        return self.model.run(image_path, text_prompt, output_filename, output_dir, box_threshold, text_threshold, flag_segmentation_image_debug)
...
class FruitNerfDataset(ColmapConverterToNerfstudioDataset):
    ...
    def main(self) -> None:
        ...
        if self.camera_type == "equirectangular":
            ...
            self.data = equirect_utils.generate_planar_projections_from_equirectangular(self.data, pers_size, self.images_per_equirect, crop_factor=self.crop_factor)
            self.camera_type = "perspective"
        summary_log = []
        if not self.skip_image_processing:
            image_rename_map_paths = process_data_utils.copy_images(self.data, image_dir=self.image_dir, crop_factor=self.crop_factor, verbose=self.verbose)
            ...
            if isinstance(self.data_semantic, Path):
                semantics_rename_map_paths = process_data_utils.copy_images(self.data_semantic, image_dir=self.semantic_dir, crop_factor=self.crop_factor, verbose=self.verbose)
            else:
                ...
                for image_path in image_rename_map_paths:
                    mask = segmentor.run(image_path=image_path, text_prompt=self.segmentation_class, output_filename=image_rename_map_paths[image_path],
                                         output_dir=self.semantic_dir, text_threshold=self.text_threshold, box_threshold=self.box_threshold,
            ...
            summary_log.append(process_data_utils.downscale_images(self.semantic_dir, self.num_downscales, folder_name=self.semantic_dir.name, verbose=self.verbose))
        else:
        ...
        summary_log += self._save_transforms(num_frames, image_id_to_depth_path, None, image_rename_map=image_rename_map)
