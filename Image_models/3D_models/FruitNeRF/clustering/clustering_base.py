...
def check_point_cloud_for_spherical_shape(point_cloud):
    ...
    spherical_shape = abs(proportion_variation_1 - 0.33333) <= 0.1 and abs(proportion_variation_2 - 0.33333) <= 0.1 and abs(proportion_variation_3 - 0.33333) <= 0.1
...
class FruitClustering(object):
    def __init__(self, voxel_size_down_sample: float = 0.00005, remove_outliers_nb_points: int = 100, remove_outliers_radius: float = 0.01, cluster_merge_distance: float = 0.04):
    ...
    def cluster(self, pcd, **kwargs):
        ...
        if np.asarray(self.pcd_downsampled_cleaned.points).shape[0] == 0:
            ...
            labels = -1
            return X, C, labels
        o3d.io.write_point_cloud(str(Path(self.pcd_path).parents[0] / 'semantic_cleaned_down_sampled.ply'), self.pcd_downsampled_cleaned)
        X, C = self.pcd2points_and_color(pcd=self.pcd_downsampled_cleaned)
        clustering = DBSCAN(eps=kwargs['eps'], min_samples=kwargs['min_sampled'], n_jobs=-1).fit(X)
        labels = clustering.labels_
        cluster_pcd = self.visualize_clusters(X=X, labels=labels, visualize=False)
        o3d.io.write_point_cloud(str(Path(self.pcd_path).parents[0] / 'semantic_cleaned_down_sampled_cluster.ply'), cluster_pcd)
        return X, C, labels
    ...
    def split_large_cluster(self, X, C, labels):
        def one_apple_cluster(cluster):
            ...
            reg_p2p = o3d.pipelines.registration.registration_icp(self.fruit_template, cluster, 0.01, t_init,
        ...
        class ClusterThread(Thread):
            ...
            def run(self):
                ...
                for cluster_idx in np.unique(l):
                    ...
                    apples = apples + apple
                dist = hausdorff_distance(np.asarray(self.cluster.points), np.asarray(apples.points), distance="euclidean")
                self.dist = dist
        ...
        for cluster in tqdm(X, desc='Large Fruit Clusters processed'):
            ...
            if self.fruit_alpha_shape_.volume < 0.9 * alpha_shape_volume.volume:
                ...
                cluster_apples = [cluster_apples_1, cluster_apples_2, cluster_apples_3, cluster_apples_4, cluster_apples_5, cluster_apples_6]
                additional_count += (apple_in_cluster - 1)
        ...
        if not isinstance(self.gt_cluster, type(None)):
            ...
            if self.gt_cluster:
                ...
                for cluster, cluster_pcd in zip(cluster_centers, cluster_single):
                    ...
                    if distance[nearest_center_id] < 0.15:
        ...
        if self.gt_cluster:
            ...
            number_of_gt_objects = self.gt_count
