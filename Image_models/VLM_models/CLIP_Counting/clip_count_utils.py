...
def object_name(key):
    return OBJ_NAMES.get(key, str(key).replace("_", " "))
NUMBER_WORDS_SUB = [
        "two", "three", "four", "five",
    ]
NUMBER_WORDS = [
...
def run_on_my_data_img_retrievel(model,processor,target_data,target,ref,normalize,device,factor,sample_size=10,num_classes=4,linear_shift=True,start_with_target_with_num=False,normalize_before_scoring=False):
    ref_aug_sentences=[f"{word} {object_name(ref)}" for word in NUMBER_WORDS[:num_classes]]
    target_aug_sentences=[f"{word} {object_name(target)}" for word in NUMBER_WORDS[:num_classes]]

    ref_diff,ref_prompt_single=get_ref_difference(
        ref_aug_sentences=ref_aug_sentences,
        ref_object=[object_name(ref)],
        model=model,
        processor=processor,
        device=device,
        normalize=normalize
    ...
    target_text_sample["target_obj_embeds"],target_text_sample["target_obj_aug_embeds"]=get_target_rep(
        target_object=[object_name(target)],
