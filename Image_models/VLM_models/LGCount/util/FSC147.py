...
class FSC147(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        class_text = self.class_dict[im_id]
        coarse_text_interval=['zero', 'one hundred','two hundred','five hundred','one thousand','infinity']
        coarse_text_interval_integer=[0, 100, 200, 500, 1000, float('inf')]
        fine_text_interval=[
        ...
        fine_text_interval_integer=[
                                    [0, 20, 40, 60, 80, 100],
                                    [100, 120, 140, 160, 180, 200],
                                    [200, 250, 300, 350, 400, 500],
                                    [500, 600, 700, 800, 900, 1000],
                                    [1000, 1200, 1400, 1800, 2500, float('inf')]
                                    ]
        ...
        if self.split == 'train' or (self.split == 'val' and self.resize_val):
            ...
            # 密度图求和，向上取整作为GT的数量
            rank_text_GT_count=math.ceil(torch.sum(sample['gt_density']/SCALE_FACTOR).item())
            coarse_GT=np.searchsorted(coarse_text_interval_integer, rank_text_GT_count, side='left')-1
            fine_GT=np.searchsorted(fine_text_interval_integer[coarse_GT], rank_text_GT_count, side='left')-1
            for i in range(len(coarse_text_interval)-1):
                coarse_text_list.append('There are between '+coarse_text_interval[i]+' and '+coarse_text_interval[i+1] +' '+class_text)
                fine_text_list_temp=[]
                for j in range(len(coarse_text_interval)-1):
                    fine_text_list_temp.append('There are between '+fine_text_interval[i][j]+' and '+fine_text_interval[i][j+1] +' '+class_text)
                fine_text_list.append(fine_text_list_temp)
            
            if self.use_additional_prompt:
                # 默认为真
                return sample['image'].float(), sample['gt_density'], sample['boxes'], sample['m_flag'], class_text, additional_prompt, coarse_text_list, fine_text_list, coarse_GT, fine_GT, im_id
            return sample['image'].float(), sample['gt_density'], sample['boxes'], sample['m_flag'], class_text, text_list, rank_text_mask, im_id
            ...
            # 密度图求和，向上取整作为GT的数量
            rank_text_GT_count=math.ceil(torch.sum(sample['gt_map']/SCALE_FACTOR).item())
            coarse_GT=np.searchsorted(coarse_text_interval_integer, rank_text_GT_count, side='left')-1
            fine_GT=np.searchsorted(fine_text_interval_integer[coarse_GT], rank_text_GT_count, side='left')-1
            coarse_text_list=[]
            fine_text_list=[]
            for i in range(len(coarse_text_interval)-1):
                ...
                for j in range(len(coarse_text_interval)-1):
                    fine_text_list_temp.append('There are between '+fine_text_interval[i][j]+' and '+fine_text_interval[i][j+1] +' '+class_text)
                fine_text_list.append(fine_text_list_temp)
            return sample['image'].float(), sample['gt_map'], sample['boxes'], sample['pos'], class_text, coarse_text_list, fine_text_list,im_id
