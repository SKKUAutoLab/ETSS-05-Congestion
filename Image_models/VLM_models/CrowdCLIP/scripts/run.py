...
def main(cfg: DictConfig):
    ...
    runner = None
    regression_datamodule = RegressionDataModule(**OmegaConf.to_container(cfg.data_cfg))    #加载数据集
    # Training
    if not cfg.test_only:
        runner = Runner(**OmegaConf.to_container(cfg.runner_cfg))           #加载模型

        logger.info("Start training.")
        with torch.no_grad():
            trainer.fit(model=runner, datamodule=regression_datamodule)     #训练
    ...
    for ckpt_path in ckpt_paths:
        ...
        runner = runner.load_from_checkpoint(str(ckpt_path), **OmegaConf.to_container(cfg.runner_cfg))  #加载预训练参数

        trainer.test(model=runner, datamodule=regression_datamodule)    #测试
