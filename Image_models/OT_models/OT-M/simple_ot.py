...
def softmin(logB, G, C, epsilon): # [1, 190, 1], [1, 190, 1], [1, 843, 190], 16757.0
    B = C.shape[0]
    x = logB.view(B, 1, -1) + (G.view(B, 1, -1) - C) / epsilon # [1, 843, 190]
    x = - epsilon * x.logsumexp(2).view(B, -1, 1) # [1, 843, 1]
    return x
...
class SampleOT:
    ...
    @torch.no_grad()        
    def __call__(self, A, B, cost, F=None, G=None, diameter=None): # [1, 843, 1], [1, 190, 1], [1, 843, 190], None, None, None
        ...
        F, G = factor * softmin(logB, G, Cab, self.blur), factor * softmin(logA, F, Cba, self.blur) # [1, 843, 1], [1, 190, 1]
    ...
    def plan(self, A, B, F, G, cost): # [1, 843, 1], [1, 190, 1], [1, 843, 1], [1, 190, 1], [1, 843, 190]
        PI1 = torch.exp((F + G.permute(0, 2, 1) - cost) / self.blur) # [1, 843, 190]
        PI2 = A * B.permute(0, 2, 1) # [1, 843, 190]
        PI = PI1 * PI2 # [1, 843, 190]
        return PI