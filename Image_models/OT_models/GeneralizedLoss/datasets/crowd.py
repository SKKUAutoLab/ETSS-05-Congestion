...
class Crowd(data.Dataset):
    def __init__(self, root_path, crop_size, downsample_ratio, is_gray=False, method='train', resize=False, im_list=None, noise=0):
        self.noise = noise
    ...
    def train_transform_with_crop(self, img, keypoints):
        ...
        if len(keypoints) < 1:
            if random.random() > 0.5:
                img = F.hflip(img)
            return self.trans(img), torch.from_numpy(keypoints.copy()).float(), torch.from_numpy(keypoints.copy()).float(), st_size
