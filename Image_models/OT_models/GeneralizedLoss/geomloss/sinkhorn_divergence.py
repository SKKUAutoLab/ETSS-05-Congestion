...
def epsilon_schedule(p, diameter, blur, scaling):
    ε_s = [ diameter**p ] + [ np.exp(e) for e in np.arange(p*np.log(diameter), p*np.log(blur), p*np.log(scaling)) ] + [ blur**p ]
    return ε_s
    ...
    if potentials:
        ...
        if debias:
            if ρ is None:
                ...
                return scal( α, UnbalancedWeight(ε, ρ)( (-a_x/ρ).exp() - (-b_x/ρ).exp() ), batch=batch ) + scal( β, UnbalancedWeight(ε, ρ)( (-b_y/ρ).exp() - (-a_y/ρ).exp() ), batch=batch ),b_x, a_y
        else:
            if ρ is None:
                return scal( α, b_x, batch=batch ) + scal( β, a_y, batch=batch ), b_x , a_y 
            else:
                return scal( α, UnbalancedWeight(ε, ρ)( 1 - (-b_x/ρ).exp() ), batch=batch ) + scal( β, UnbalancedWeight(ε, ρ)( 1 - (-a_y/ρ).exp() ), batch=batch ), b_x , a_y

def sinkhorn_loop( softmin, α_logs, β_logs, C_xxs, C_yys, C_xys, C_yxs, ε_s, ρ, jumps=[], kernel_truncation=None, truncate=5, cost=None, extrapolate=None, debias=True, last_extrapolation=True ):
    ...
    if debias:
        a_x = λ * softmin(ε, C_xx, α_log )
        b_y = λ * softmin(ε, C_yy, β_log )
    a_y = λ * softmin(ε, C_yx, α_log )
    b_x = λ * softmin(ε, C_xy, β_log )
    for i, ε in enumerate(ε_s):
        λ = dampening(ε, ρ)
        if debias:
            at_x = λ * softmin(ε, C_xx, α_log + a_x/ε )
            bt_y = λ * softmin(ε, C_yy, β_log + b_y/ε )
        at_y = λ * softmin(ε, C_yx, α_log + b_x/ε )
        bt_x = λ * softmin(ε, C_xy, β_log + a_y/ε )
        if debias:
            a_x, b_y = .5 * ( a_x + at_x ), .5 * ( b_y + bt_y )
        a_y, b_x = .5 * ( a_y + at_y ), .5 * ( b_x + bt_x )
        if i in jumps:
            if i == len(ε_s) - 1:
                if debias:
                ...
                if debias:
                    C_xx_, _     = kernel_truncation( C_xx, C_xx, C_xxs[k+1], C_xxs[k+1], a_x, a_x, ε, truncate=truncate,cost=cost)
                    C_yy_, _     = kernel_truncation( C_yy, C_yy, C_yys[k+1], C_yys[k+1], b_y, b_y, ε, truncate=truncate,cost=cost)
                C_xy_, C_yx_ = kernel_truncation( C_xy, C_yx, C_xys[k+1], C_yxs[k+1], b_x, a_y, ε, truncate=truncate,cost=cost)
            if debias:
