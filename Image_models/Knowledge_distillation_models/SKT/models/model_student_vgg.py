...
channel_nums = [[32, 64, 128, 256], [21, 43, 85, 171], [16, 32, 64, 128], [13, 26, 51, 102]]
...
class CSRNet(nn.Module):
    ...
    def forward(self, x): # [1, 3, 509, 766]
        self.features = []
        x = self.conv0_0(x)
        if self.transform:
            self.features.append(self.transform0_0(x)) # [1, 64, 509, 766]
        x = self.conv0_1(x)
        x = self.pool0(x)
        if self.transform:
            self.features.append(self.transform1_0(x)) # [1, 64, 255, 383]
        x = self.conv1_0(x)
        x = self.conv1_1(x)
        x = self.pool1(x)
        if self.transform:
            self.features.append(self.transform2_0(x)) # [1, 128, 128, 192]
        x = self.conv2_0(x)
        x = self.conv2_1(x)
        x = self.conv2_2(x)
        x = self.pool2(x)
        if self.transform:
            self.features.append(self.transform3_0(x)) # [1, 256, 64, 96]
        x = self.conv3_0(x)
        x = self.conv3_1(x)
        x = self.conv3_2(x)
        x = self.conv4_0(x)
        if self.transform:
            self.features.append(self.transform4_0(x)) # [1, 512, 64, 96]
        x = self.conv4_1(x)
        x = self.conv4_2(x)
        x = self.conv4_3(x)
        if self.transform:
            self.features.append(self.transform4_3(x)) # [1, 256, 64, 96]
        x = self.conv4_4(x)
        x = self.conv4_5(x)
        x = self.conv5_0(x) # [1, 1, 64, 96]
...
def conv_layers(inp, oup, dilation=False):
    if dilation:
        ...
        d_rate = 1
    return nn.Sequential(nn.Conv2d(inp, oup, kernel_size=3, padding=d_rate, dilation=d_rate), nn.ReLU(inplace=True))
